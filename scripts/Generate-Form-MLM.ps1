<#
.SYNOPSIS
  Generates a form MLM template.
.DESCRIPTION
  This script generates a template for a form-triggered medical logic module (MLM)
  based on user input. Form MLMs are used in order forms.
#>
...
# Build the outputs.
$title = (Get-Culture).TextInfo.ToTitleCase($form + ' ' + $function)
...
knowledge:
	...
	;;
	data:
		(communication, form, client) := argument;
	;;
	priority:
		50
	;;
